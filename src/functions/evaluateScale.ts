...
export function evaluateScale(cards: string[]): Scale[] {
	...
	const cardsIndices = [];
	cards.forEach(x => cardsIndices.push(DECK.find(y => y.sign === x).scalePriority));
	...
	const DECK_SIGNS = DECK.map(x => x.sign);
	for (const s of scalesInternal) {
		if (s.color === null) {
			...
			const curScale = new Scale(DECK_SIGNS[s.color*8][0] + '-count-' + s.count + '-upto-' + DECK_SIGNS[s.top][1],
										c.points,
